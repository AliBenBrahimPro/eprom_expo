...
const ListConfiguration = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        const response = await axios.get("http://10.0.2.2:8085/api/type-questions", {
          headers: {
  ...
  const renderSurveyItem = ({ item }) => (
    ...
    >
      ...
      <Text style={{ fontSize: 14, color: "#888" }}>
        Date Created:{" "}
        {moment(item?.createdDate).format("MMMM D, YYYY")}
      </Text>
  ...
  const handleEdit = (item) => {
    ...
    navigation.navigate("EditConfiguration", { itemId: item.id });
  };
  ...
  const deleteSurvey = async (surveyId) => {
    try {
      ...
      // Add the token to the request headers
      await axios.delete(`http://10.0.2.2:8085/api/type-questions/${surveyId}`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
