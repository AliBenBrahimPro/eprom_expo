...
import { View, Text, FlatList, TouchableOpacity, Modal, Button,Alert ,StyleSheet} from 'react-native';
...
const ListSurveys = () => {
  ...
  const renderSurveyItem = ({ item }) => (
    ...
    <Text style={{ fontSize: 18, fontWeight: 'bold', marginBottom: 5 }}>Title: {item.title}</Text>
    <Text style={{ fontSize: 16, marginBottom: 5 }}>Topic: {item.topic}</Text>
    <Text style={{ fontSize: 16, marginBottom: 5 }}>Number of Questions: {item.questions.length}</Text>
  ...
  const handleInvite = (item) => {
    navigation.navigate('InviteSurveys', { survey_id: item.id });

  };

  const handleEdit = (item) => {
    // Navigate to the EditStudy screen with the study details
    navigation.navigate('EditSurveys', { itemId: item.id });
  };
  ...
  const deleteSurvey = async (surveyId) => {
    try {
      ...
      // Add the token to the request headers
      await axios.delete(`http://10.0.2.2:8085/api/surveys/${surveyId}`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
      ...
      setData((prevData) => prevData.filter((survey) => survey.id !== surveyId));
    } catch (error) {
 ...
 <View style={styles.modalContainer}>
  <View style={styles.modalContent}>
    ...
    <Text>Title: {selectedStudy?.title}</Text>
    <Text>Topic: {selectedStudy?.topic}</Text>
    <Text>Topic: {selectedStudy?.description}</Text>
